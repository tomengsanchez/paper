...
<body>
    <aside class="sidebar">
        ...
        <nav class="py-2">
            <a href="<?= BASE_URL ?>/" class="<?= ($currentPage ?? '') === 'dashboard' ? 'active' : '' ?>">Dashboard</a>
            <?php if (\Core\Auth::can('view_items')): ?>
            <a href="<?= BASE_URL ?>/item" class="<?= ($currentPage ?? '') === 'items' ? 'active' : '' ?>">Items (example)</a>
            <?php endif; ?>
            ...
            <div class="nav-sub">
                ...
                <a href="<?= BASE_URL ?>/users/profiles" class="<?= ($currentPage ?? '') === 'user-profiles' ? 'active' : '' ?>">User Profiles</a>
                <?php endif; ?>
                <?php if (\Core\Auth::can('view_users')): ?>
                <a href="<?= BASE_URL ?>/users" class="<?= ($currentPage ?? '') === 'users' ? 'active' : '' ?>">Users</a>
                <?php endif; ?>
                <?php if (\Core\Auth::can('view_roles')): ?>
                <a href="<?= BASE_URL ?>/users/roles" class="<?= ($currentPage ?? '') === 'user-roles' ? 'active' : '' ?>">Roles &amp; Capabilities</a>
                <?php endif; ?>
            </div>
            <?php endif; ?>
            ...
            <div class="nav-sub">
                ...
                <a href="<?= BASE_URL ?>/settings" class="<?= ($currentPage ?? '') === 'settings' ? 'active' : '' ?>">General</a>
                <?php endif; ?>
                <?php if (\Core\Auth::can('view_email_settings')): ?>
                <a href="<?= BASE_URL ?>/settings/email" class="<?= ($currentPage ?? '') === 'email-settings' ? 'active' : '' ?>">Email</a>
                <?php endif; ?>
                <?php if (\Core\Auth::can('view_security_settings')): ?>
                <a href="<?= BASE_URL ?>/settings/security" class="<?= ($currentPage ?? '') === 'security-settings' ? 'active' : '' ?>">Security</a>
                <?php endif; ?>
            </div>
            <?php endif; ?>
        </nav>
    ...
    <script>$(function(){ $('.nav-parent').on('click', function(){ $(this).toggleClass('open'); }); });</script>
